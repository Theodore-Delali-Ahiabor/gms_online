                ...
                if(empty($category) || empty($make) || empty($color) || empty($fuel) || empty($year) || empty($vin)){
                    ...
                    if($row['numrows'] > 0){
                        ...
                        try {
                            ...
                            $stmt = $conn->prepare("INSERT INTO `automobiles`(`Photo`, `CategoryID`, `Year`, `MakeID`, `Model`, `Color`, `FuelID`, `VIN`, `RegNumber`) 
                            VALUES (:photo, :category, :year, :make, :model, :color, :fuel, :vin, :registration)");
                            $stmt->execute(['photo'=>$photo ,'category'=>$category ,'year'=>$year ,'make'=>$make ,'model'=>$model ,'color'=>$color ,'fuel'=>$fuel, 'vin'=>$vin, 'registration'=>$registration]);
                            $output['type'] = 'success';
